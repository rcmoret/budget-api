// forked from design-sync lib/source-kit.mjs — group comes from .design-sync/docs
// frontmatter, not from directory names (see the group block near the end).
//
// Why: this repo's component directories are implementation folders (card/,
// cta/, adjustment-input/, left-column/), not design-system categories. Upstream
// only lets a doc's `category:` win when the dir-derived group is already
// generic, which split sibling components across "cta" and "actions". Yielding
// 'general' here makes the docs the single source of grouping truth.
//
...
export async function resolvePackage(ctx) {
  ...
  if (srcRoot) {
    for (const c of components) {
      ...
      // FORK: upstream derives the group from the last non-generic src/ path
      // segment. Here every component directory is an implementation folder
      // rather than a category, so dir-derived groups fight the taxonomy in
      // .design-sync/docs. Yield 'general' (still honouring an explicit JSDoc
      // @category) so package-build.mjs lets the doc's `category:` win for
      // every component, not just the ones that happened to sit at the root.
