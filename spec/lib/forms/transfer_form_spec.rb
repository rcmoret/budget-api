...
RSpec.describe Forms::TransferForm do
  ...
  describe "validations" do
    context 'when the "to" account is not present' do
      ...
      before do
        allow(Account).to receive(:fetch).with(user,
          key: to_account.key).and_return(nil)
        allow(Account).to receive(:fetch).with(user,
          key: from_account.key).and_call_original
      end
      ...
      it "returns an error tuple" do
        expect(subject.call).to match(
          [
            :error,
            hash_including(to_account: [ "can't be blank" ]),
          ]
        )
      end
    end
    ...
    context 'when the "from" account is not present' do
      ...
      before do
        allow(Account).to receive(:fetch).with(user,
          key: to_account.key).and_return(to_account)
        allow(Account).to receive(:fetch).with(user,
          key: from_account.key).and_return(nil)
      end
      ...
      it "returns an error tuple" do
        expect(subject.call).to match(
          [
            :error,
            hash_including(from_account: [ "can't be blank" ]),
          ]
        )
      end
    end
        ...
        expect(subject.call)
          .to eq(
            [
              ...
              {
                from_account: [
                  "cannot be the same account as 'to' account",
                ],
              },
            ]
          )
    ...
    context "when the amount is zero" do
      ...
      it "returns an error tuple" do
        expect(subject.call).to match(
          [
            :error,
            hash_including(amount: [ "must be greater than 0" ]),
          ]
        )
      end
    end
    ...
    context "when the amount is a negative number" do
      ...
      it "creates a new transaction entry for the to account" do
        expect { subject.call }.to change {
          Transaction::Entry.where(account_id: to_account.id).count
        }.by(+1)
      end

      it "creates a new transaction entry for the from account" do
        expect { subject.call }.to change {
          Transaction::Entry.where(account_id: from_account.id).count
        }.by(+1)
      end
