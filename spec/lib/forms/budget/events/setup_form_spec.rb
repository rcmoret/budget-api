...
RSpec.describe Forms::Budget::Events::SetupForm do
  ...
  describe "interval not setup validation" do
    before do
      allow(Forms::Budget::EventsForm)
        .to receive(:new)
        .and_return(instance_double(Forms::Budget::EventsForm, valid?: true,
          save: true))
    end
  ...
  describe "events form validation" do
    ...
    let(:error_double) do
      instance_double(ActiveModel::Error, attribute: "event_type",
        full_message: "No registered handler")
    end
    let(:events_form_double) do
      instance_double(Forms::Budget::EventsForm, valid?: false,
  ...
  context "when the form does not save correctly" do
    ...
    let(:error_double) do
      instance_double(ActiveModel::Error, attribute: "create.0.amount",
        full_message: "Can't be blank")
    end
    let(:form_double) do
      instance_double(Forms::Budget::EventsForm, valid?: true, save: false,
        errors: errors_double)
    end
    let(:events_params) { [ { event_type: valid_create_event } ] }
  ...
  describe "updates to the interval" do
    ...
    before do
      ...
      allow(Forms::Budget::EventsForm)
        .to receive(:new)
        .and_return(instance_double(Forms::Budget::EventsForm, save: true,
          valid?: true))
    end
    ...
    let(:interval) do
      create(:budget_interval, user_group:, month:, year:)
    end
    let(:events_params) { [ { event_type: valid_create_event } ] }
    ...
    context "when the start and end dates are not populated " \
            "and no info is passed in" do
  ...
  describe "initializing and saving the events form" do
    ...
    let(:form_double) do
      instance_double(Forms::Budget::EventsForm, valid?: true, save: true)
    end
    let(:events_params) { [ { event_type: valid_create_event } ] }
    let(:interval) { create(:budget_interval) }
