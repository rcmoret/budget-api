...
RSpec.describe WebApp::Budget::Interval::DiscretionarySerializer do
  ...
  shared_context "with a stubbed out detail serializer" do
    let(:detail_serializer) do
      instance_double(WebApp::Budget::Interval::TransactionDetailSerializer)
    end

    before do
      allow(WebApp::Budget::Interval::TransactionDetailSerializer)
        .to receive(:new)
  ...
  shared_context "when there is a pending details " \
                 "and the interval is current" do
    let(:user) { create(:user) }
    let(:interval) { create(:budget_interval, user_group: user.group) }
  ...
  shared_context "when there is a pending details and " \
                 "the interval is not current" do
    let(:user) { create(:user) }
  ...
  describe "#transaction_details" do
    ...
    context "when there is a pending details and the interval is current" do
      include_context "when there is a pending details " \
                      "and the interval is current"
      include_context "with a stubbed out detail serializer"
    ...
    context "when there is a pending details and the interval is not current" do
      include_context "when there is a pending details and " \
                      "the interval is not current"
  ...
  describe "#over_under_budget" do
    ...
    let(:monthly_item) do
      create(:budget_item, interval:, category: monthly_category)
    end
    ...
    let(:day_to_day_category) do
      create(:category, :weekly, user_group: user.group)
    end
    let(:day_to_day_item) do
      create(:weekly_item, interval:, category: day_to_day_category)
    end
    let(:day_to_day_presenter) do
      instance_double(Presenters::Budget::DayToDayExpensePresenter,
        budget_impact: -50_80)
    end
    ...
    it "returns a sum of the items' budget impact" do
      [ day_to_day_presenter,
        monthly_presenter, ].map(&:budget_impact).sum.then do |expected_amount|
  ...
  describe "#amount" do
    ...
    let(:account) { create(:account, user_group: user.group) }
    let(:savings_account) { create(:savings_account, user_group: user.group) }
    let(:weekly_expense) { create(:weekly_expense, interval:) }
    let(:weekly_expense_double) do
      instance_double(Presenters::Budget::DayToDayExpensePresenter,
        remaining: -143_00)
    end
    let(:monthly_expense) { create(:monthly_expense, interval:) }
    let(:monthly_expense_double) do
      instance_double(Presenters::Budget::MonthlyItemPresenter,
        remaining: -13_40)
    end
    let(:remaining) do
      [ weekly_expense_double, monthly_expense_double ].map(&:remaining).sum
    end
    ...
    context "when future" do
      ...
      context "when no budget inclusions in non-cash-flow accounts, " \
              "no pending transactions" do
      ...
      context "when there are cleared budget inclusions " \
              "in non-cash-flow accounts" do
        let!(:current_transaction) do
          create(
            :transaction_entry,
            account: savings_account,
    ...
    context "when current" do
      ...
      context "when no budget inclusions in non-cash-flow accounts, " \
        ...
        it "returns the remaining and balance prior to the " \
           "last date of the cash flow accounts" do
          expect(subject.amount)
            .to be(
              remaining +
              current_transaction.total +
      ...
      context "when no budget inclusions in non-cash-flow accounts, " \
        ...
        it "returns the sum of: remaining, the cash flow balance " \
           "prior to the last date" do
          expect(subject.amount)
            .to be(
              remaining +
              existing_cash_flow_transaction.total +
              pending_transaction.total
            )
        end
      end

      context "when there are some cleared budget inclusions " \
              "in non-cash-flow accounts" do
        ...
        it "returns the sum of: remaining, cash flow balance, " \
           "cleared budget inclusions" do
          expect(subject.amount)
            .to be(
              remaining +
              existing_cash_flow_transaction.total +
      ...
      context "when there are some pending budget inclusions " \
              "in non-cash-flow accounts" do
        ...
        it "returns the sum of remaining, cash flow accounts balance, " \
           "pending budget inclusions" do
          expect(subject.amount)
            .to be(
              remaining +
              existing_cash_flow_transaction.total +
    ...
    context "when in the past" do
      ...
      context "when no budget inclusions in non-cash-flow accounts, " \
        ...
        it "returns the remaining plus existing cash flow" do
          expect(subject.amount)
            .to be(remaining + existing_cash_flow_transaction.total)
        end
      end
      ...
      context "when there are some pending budget inclusions in " \
              "non-cash-flow accounts" do
        before do
          create(
            :transaction_entry,
            :budget_exclusion,
