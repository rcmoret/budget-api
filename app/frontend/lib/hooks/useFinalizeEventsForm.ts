...
const decorate = (props: {
  ...
  const appliedToExtra =
    showWarning || !rolloverAmount.display
      ? 0
...
const useFinalizeEventsForm = (props: HookProps) => {
  ...
  const [categories, setCategories] = useState<FinalizeFormCategory[]>(
    mapCategories({
  ...
  const extraAmount = categories.reduce((total, { items }) => {
    return (
      total + items.reduce((sum, { appliedToExtra }) => sum + appliedToExtra, 0)
    );
  }, 0);

  const categoryEventsReducer = ({
    items,
    events,
  }: FinalizeFormCategory): EventProps[] => {
    return events.reduce((collection, event) => {
      const relevantItems = items.filter((i) => i.eventKey === event.key);
      const adjustAmount = relevantItems.reduce(
        (sum, item) => sum + (item.rolloverAmount.cents || 0),
        0,
      );
      if (!adjustAmount) {
        return collection;
  ...
  useEffect(() => {
    if (allItemsReviewed) {
      const excludedKeys = allItemsReviewed
        ? events
            .filter((ev) => "budgetCategoryKey" in ev)
            .map((ev) => ev.budgetCategoryKey)
        : [];
  ...
  const submittableEvents: EventProps[] = [
    {
      ...
      budgetCategoryKey:
        extraEvent?.budget_category_key ?? generateKeyIdentifier(),
      budgetItemKey: generateKeyIdentifier(),
      month,
      year,
      data: {},
