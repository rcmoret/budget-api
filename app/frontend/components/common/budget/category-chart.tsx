...
const MonthlyDataChart = (props: { data: Array<MonthlyData> }): JSX.Element | null => {
    ...
    <div className="w-full flex flex-col gap-1">
      ...
      <div className="w-full flex flex-row justify-between">
        ...
        <div className="w-7/12 text-xs flex flex-row justify-between border-b border-blue-200">
          ...
          <div>
            <AmountSpan amount={maxAmount / 2} absolute={true} showCents={false} />
          </div>
          <div>
      ...
      {props.data.map((monthlyData) => (
        <Bar key={`${monthlyData.year}.${monthlyData.month}`} maxAmount={maxAmount} {...monthlyData} />
      ))}
    </div>
  );
};
...
const getSummaryData = async (props: {
  ...
  axios.get<CategorySummaryResponse>(summaryUrl)
    .then(response => {
...
type TCategoryAverageComponentProps<T extends GenericCategory = GenericCategory> = {
  month: number;
  year: number;
  category: T;
};
...
const CategoryAverageContext = createContext<TCategoryAverageComponentProps | null>(null);

type TCategoryAverageContext<T extends GenericCategory = GenericCategory> = TCategoryAverageComponentProps<T> & {
  addSummaryData: (props: { key: string, data: TBudgetCategorySummary }) => void;
...
const useCategoryAveragesContext = <T extends GenericCategory = GenericCategory>(): TCategoryAverageContext<T> => {
  const context = useContext(CategoryAverageContext);
  if (!context) {
    throw new Error("useCategoryAveragesContext must be used within a Category Average Provider");
  }
...
const CategoryAveragesProvider = <T extends GenericCategory>(props: {
  ...
  return (
    <CategoryAverageContext.Provider value={value as TCategoryAverageComponentProps<T>}>
      {children}
    </CategoryAverageContext.Provider>
  );
};

export { SummaryUpdateButton, CategoryAverages, useCategoryAveragesContext, CategoryAveragesProvider }
