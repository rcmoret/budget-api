...
const MonthYearSelect = (props: ComponentProps) => {
  ...
  const update = (payload: UpdateProps) => updateState({
    ...state,
    ...payload,
  });
  const toggleForm = () => update({
...
const Form = (props: FormProps) => {
  ...
  const handleInputChange = (event: React.ChangeEvent & { target: HTMLInputElement }) => {
    const target = event.target;
    update({
      [target.name]: String(target.value),
    });
  };
  const handleSelectChange = (event: { label: string, value: string }) => update({
