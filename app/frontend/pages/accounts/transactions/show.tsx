...
const TransactionShow = (props: { transaction: ModeledTransaction, balance: number }) => {
  ...
  const clearanceDate = isPending ? "Pending" : dateParse(String(transaction.clearanceDate));
  const shortClearanceDate = isPending ? "Pending" : dateParse(String(transaction.clearanceDate), {
    format: "m/d/yy",
  });
  ...
  return (
    <TransactionContainer
      ...
      transactionAmountComponent={isDetailShown ?
    ...
    >
      <Cell styling={{
        ...
        {!!description && details.length > 1 && <BudgetItemsDescription details={details} />}
        {isBudgetExclusion &&
          <div className="ml-4 md:max-w-2/12 max-md:w-full italic">budget exclusion</div>}
        {checkNumber &&
          <div className="ml-4">
...
const BudgetItemsDescription = (props: { details: AccountTransactionDetail[] }) => {
  const details = props.details.filter((detail: AccountTransactionDetail) => detail.budgetCategoryName).sort(byCategoryName);
...
const BudgetItemList = (props: { details: AccountTransactionDetail[] }) => props.details.sort(byAmount).map(detail => (
...
const BudgetItemAmounts = (props: { details: AccountTransactionDetail[], amount: number }) => (
  <div className="w-full">
