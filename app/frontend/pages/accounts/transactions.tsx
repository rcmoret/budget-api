...
const TransferComponent = () => {
  ...
  if (!account.showTransferForm) {
    return;
  }

  const { slug, key } = account;
  const options = accounts.reduce(
    (collection, account) => {
      if (key === account.key) {
        return collection;
      } else {
  ...
  const value = options.find((option) => option.value === data.accountKey) || {
    label: "",
    value: "",
  };
  ...
  return (
    <div className="w-full py-2 px-4 bg-sky-100 flex flex-row gap-8">
      ...
      <div className="min-w-72">
        <Select options={options} onChange={onAccountChange} value={value} />
      </div>
      <div>
        <AmountInput
      ...
      <div>
        ...
        >
          <div className="flex flex-row gap-2">
            ...
            <div
              className={
                isSubmittable ? "text-chartreuse-300" : "text-gray-800"
              }
            >
...
const Transactions = (props: ComponentProps) => {
  ...
  const filteredTransactions = transactions.reduce((acc, txn) => {
    ...
    const detailDescriptions = details.map(
      (detail) => detail.budgetCategoryName,
    );
    if (
      !!String(description).match(filterExpression) ||
      detailDescriptions.some((d) => !!d?.match(filterExpression))
    ) {
      return [
        {
          ...txn,
  ...
  const displayableTransactions = filterActive
