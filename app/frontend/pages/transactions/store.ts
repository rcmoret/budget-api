...
const initTransactionIndexStore = (props: Pick<TransactionsIndexState, "accounts" | "budgetItems" | "featuredAccount" | "transactions">
) => {
  const { budgetItems, accounts, featuredAccount, transactions } = props;
  const setAccounts = useTransactionsIndexStore((s) => s.setAccounts)
  const setFeaturedAccount = useTransactionsIndexStore((s) => s.setFeaturedAccount)
...
const getAccounts = () => useTransactionsIndexStore((s) => s.accounts)
const getBudgetItems = () => useTransactionsIndexStore((s) => s.budgetItems)
const getFeaturedAccount = () => useTransactionsIndexStore((s) => s.featuredAccount)
