...
const ItemDetailHistory = ({ details }: { details: Array<BudgetItemEvent | BudgetItemTransaction> }) => {
  ...
  const decoratedDetails: Array<LineItemProps> = details.map((detail) => {
    ...
    const remaining = item.isExpense ? Math.max(0, difference) : Math.min(0, difference)
