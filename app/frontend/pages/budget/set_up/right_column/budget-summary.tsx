...
const BudgetSummary = () => {
  ...
  const {
    revenues: { metadata: { isSelected: revenuesSelected } },
    monthlyExpenses: { metadata: { isSelected: monthlySelected } },
    dayToDayExpenses: { metadata: { isSelected: dayToDaySelected } },
  } = groups
  ...
  return (
    <div className={`flex w-96 flex-col rounded p-2 gap-1 ${baseBg} shadow-md`}>
      <BudgetSummaryLine label="Revenues" fontSize={fontSizes.revenues} bgColor={bgs.revenues}>
        <AmountComponent amount={groups.revenues.metadata.sum} />
      </BudgetSummaryLine>
      <BudgetSummaryLine label="Monthly Expenses" fontSize={fontSizes.monthly} bgColor={bgs.monthly}>
        <AmountComponent amount={groups.monthlyExpenses.metadata.sum} />
      </BudgetSummaryLine>
      <BudgetSummaryLine label="Day to Day Expenses" fontSize={fontSizes.dayToDay} bgColor={bgs.dayToDay}>
        <AmountComponent amount={groups.dayToDayExpenses.metadata.sum} />
      </BudgetSummaryLine>
      <BudgetSummaryLine label={`${metadata.month}/${metadata.year}`} fontSize="base" showBorder={true} bgColor={baseBg}>
