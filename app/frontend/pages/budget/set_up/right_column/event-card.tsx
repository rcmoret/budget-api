...
const NewAmount = () => {
  ...
  const className = [...(!event.updatedAmount ? ["font-semibold"] : [])].join(
    " ",
  );
...
const EventCard = () => {
  const { amount, event, eventTypeLabel, index, flags, setters, updateEvent } =
  ...
  return (
    ...
    >
      <div className="w-full flex flex-row justify-between">
        ...
        <div>{event.eventType === "setup_item_create" && <DeleteButton />}</div>
      </div>
      <div className="w-full flex flex-row justify-between gap-4">
        <Suggestions />
        <div className="flex flex-col gap-2">
          {event.eventType === "setup_item_adjust" && (
            <PreviouslyBudgetedAmount />
          )}
          <AmountInput
            name="setup-event"
