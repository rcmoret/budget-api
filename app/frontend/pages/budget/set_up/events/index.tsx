...
const SetUpEventContext = createContext<TSetUpCategoryEventContext | null>(null)

const SetUpEventProvider = (props: { event: SetupEvent; index: number; children?: React.ReactNode }) => {
  ...
  const setters = {
    ...
    setBudgeted: () => {
      ...
      updateAmount({ key: event.budgetItemKey, adjustment: inputAmount({ cents: event.previouslyBudgeted }) })
    },
    setDeleteIntent: () => {
      setSelectedSuggestion("delete")
      updateAmount({ key: event.budgetItemKey, adjustment: inputAmount({ cents: (-1 * event.amount) }) })
    },
    setDefault: () => {
      setSelectedSuggestion("default")
      updateAmount({ key: event.budgetItemKey, adjustment: inputAmount({ cents: category.defaultAmount }) })
    },
    setNoAjustment: () => {
      setSelectedSuggestion("unchanged")
      updateAmount({ key: event.budgetItemKey, adjustment: inputAmount({ display: "0" }) })
    },
    setSpent: () => {
      setSelectedSuggestion("spent")
      updateAmount({ key: event.budgetItemKey, adjustment: inputAmount({ cents: event.spent }) })
    },
...
const useSetUpEventContext = (): TSetUpCategoryEventContext => {
  ...
  if (!context) {
    throw new Error("useSetUpCategoryContext must be used with a Set Up Event Provider")
  }
  return context
}
