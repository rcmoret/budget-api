...
const SelectionMarker = (props: { isSelected: boolean }) => {
  if (props.isSelected) {
    return <div className="text-xl text-chartreuse-700">&#9679;</div>;
  } else {
    return <div>&#9675;</div>;
  }
};
...
const SelectAllOption = (props: {
  ...
  return (
    <div>
      <OptionButton
        isSelected={rolloverAllSelected}
        onClick={rolloverAll}
        type="button"
      >
...
const SelectNoneOption = (props: {
  ...
  const rolloverNoneSelected =
    !rolloverAmount.cents && !!rolloverAmount.display;
  ...
  return (
    <div className="w-full">
      <OptionButton
        isSelected={rolloverNoneSelected}
        onClick={onClick}
        type="button"
      >
...
const SelectSomeIndicator = () => {
  ...
  const rolloverNoneSelected =
    !rolloverAmount.cents && !!rolloverAmount.display;
  const rolloverAllSelected = item.rolloverAmount.cents === item.remaining;
  const rolloverSomeSelected =
    !rolloverNoneSelected && !rolloverAllSelected && !!rolloverAmount.display;
...
const OptionButton = (props: OptionButtonProps | OptionIndicatorProps) => {
  ...
  if ("onClick" in props) {
    ...
    return <div className={buttonClasses.join(" ")}>{children}</div>;
  }
};
