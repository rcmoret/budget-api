import {
  ...
  defaultOutlineClasses as outlineClasses,
...
const ItemAmountForm = (props: {
  ...
  const refCallback = (element: HTMLInputElement | null) => {
    ...
    if (props.amountInputRef) {
      (
        props.amountInputRef as React.MutableRefObject<HTMLInputElement | null>
      ).current = element;
    }
...
const EventSelect = () => {
  ...
  const selectedOption =
    options.find((option) => option.value === item.eventKey) || null;
...
const ItemCard = (props: {
  ...
  const { setItemEventKey, setRolloverAmountForItem, setRolloverNoneForItem } =
    useCategory();
  const { item } = props;
  ...
  return (
    ...
    >
      <KeySpan />
      <div
        className={`flex flex-col gap-2 border ${borderColor} rounded p-4 mb-2 shadow-lg ${bgColor}`}
      >
