...
const EventItem = (props: { event: FinalizeCategoryEvent }) => {
  ...
  const singleMatched = category.items.length === 1 && category.events.length === 1
...
const EventSummary = (props: {
  event: FinalizeCategoryEvent;
}) => {
  const { event } = props
  const { category, effectiveEvents, effectItems, rolloverAmountFor } = useCategory()

  const total = rolloverAmountFor(event.key)

  if (category.items.length === 1 && effectiveEvents.length === 1 && event.eventType === "rollover_item_create") {
    return (
  ...
  if (!!items.length) {
    return (
      <div className="flex flex-col gap-2 border border-chartreuse-200 rounded p-4 mb-2 bg-chartreuse-50 shadow-lg">
        {events.map((event) => (
          <div key={event.key} className="w-full flex flex-col gap-2 my-2 px-4 py-2">
