...
const ReviewListItems = () => {
  ...
  if (
    ...
    !effectiveEvents.some((ev) => rolloverAmountFor(ev.key))
  ) {
    return null;
  } else {
    return (
...
const ReviewList = () => {
  const { categories: unsortedCategories, setCategory } =
    useFinalizeFormContext();

  const categories = unsortedCategories.sort((cat1, cat2) =>
    cat1.name < cat2.name ? -1 : 1,
  );
...
const CategoryShow = (props: { category: FinalizeFormCategory }) => {
  ...
  if (category.key !== viewingCategoryKey) {
    ...
    return (
      ...
      >
        <KeySpan _key={category.key} id={`category-${category.slug}`} />
        <CircleNavButtons
        ...
        <div className="text-lg">
          {category.name}
          {category.isAccrual && (
            <div className="text-sm text-gray-700"> (accrual)</div>
          )}
        </div>
        <div className="mb-2">
  ...
  if (allItemsReviewed) {
    return (
      <div className="flex flex-row justify-end my-2 py-2 border-t-2 border-chartreuse-600">
        <div className="flex flex-col gap-2 w-52">{props.children}</div>
      </div>
    );
  } else {
    return null;
      ...
      <>
        {groups.map(({ label, collection }) => (
          <CategoryGroup
            key={label}
            label={label}
            categories={collection.sort(byName)}
          />
        ))}
      </>
    );
