...
import { FinalizeCategoryFormItem, FinalizeFormCategory } from "@/lib/hooks/useFinalizeEventsForm";
...
const CompletionMarker = (props: { needsReview: boolean }) => {
  if (props.needsReview) {
    return (
      <div className="font-semibold text-gray-1000">
        &#9675;
      </div>
    )
  } else {
...
const CategoryButton = (props: { category: FinalizeFormCategory; isCurrentlyViewing: boolean; }) => {
  ...
  return (
    <button type="button" className={buttonClassName(props.isCurrentlyViewing)} onClick={onClick}>
...
const CategoryList = () => {
  ...
  return (
    <>
      ...
      <SummaryButton isCurrentlyViewing={viewingCategoryKey === "__summary__"} />
    </>
  )
}
