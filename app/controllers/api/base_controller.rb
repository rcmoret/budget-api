  ...
  class BaseController < ActionController::API
    ...
    define_link :check_token_presence do |*|
      if request.env["HTTP_AUTHORIZATION"].present?
        [ :ok,
          { token: request.env["HTTP_AUTHORIZATION"].match(/\ABearer (?<token>.*)\z/)[:token] }, ]
      else
        [ :error, { token: :missing } ]
      end
    end
    ...
    define_link :validate_auth_token_context do |payload|
      ...
      Auth::Token::Context.fetch(payload.fetch(:user),
        key: auth_token_key).then do |auth_token_context|
    ...
    define_link :check_ip_address do |payload|
      unless payload.fetch(:auth_token_context).ip_address == request.ip
        [ :error,
          { token: :invalid }, ]
      end
    end
